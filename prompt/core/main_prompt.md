...
"Which Notion page should I upload documents to? (e.g., '개념 노트 -> DSA')"
Store the answer as the target page for all subsequent Notion uploads in this session.
_Note that the Notion target path is independent of the local semantic directory structure (e.g., `src/concept_workspace/DSA/Algorithm/Big-O.md`). Do not force the local paths to match Notion page names._
...
0. **Troubleshooting SOP**: If any unexpected error or failure occurs, STOP. First, SEARCH `prompt/troubleshooting.md` (`<issue_index>`) for similar issues. If a match is found, apply the documented solution. If no match is found, troubleshoot the issue yourself, resolve it, and then CREATE a new `TS-[ID]` file in `docs/troubleshooting/` following the strict `<file_template>`, and prepend it to the `<issue_index>` in `prompt/troubleshooting.md`.
...
5. If Claude CLI returns an error at any step, stop the entire workflow immediately and report the error to the user. Do not proceed to the next step. Do not write the draft yourself as a fallback.
6. Always run Claude CLI in the foreground. Do not run it in the background.
